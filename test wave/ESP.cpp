...
void CEsp::offscreen(IClientEntity* entity) {
	...
	{		
		...
		Vector circlevec;
		CalcAngle69(QAngle(0, 270, 0) - newangle + QAngle(0, eyeangles.y,0), poopvec);
...
void CEsp::DrawBox(CEsp::ESPBox size, Color color)
{
	...
	// Corner Box
	int VertLine = (((float)size.w) * (255));
	int HorzLine = (((float)size.h) * (255));

	// Full Box
	Render::Outline(size.x, size.y, size.w, size.h, color);
	Render::Outline(size.x - 1, size.y - 1, size.w + 2, size.h + 2, Color(255, 255, 255, 255));
	Render::Outline(size.x + 1, size.y + 1, size.w - 2, size.h - 2, Color(255, 255, 255, 255));
}
