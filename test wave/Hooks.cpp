...
void ClanTag()
{
	...
	case 1:
	{
		...
		switch (value) {
		case 0:SetClanTag("WaveWare", ""); break;
		case 1:SetClanTag("WaveWare", ""); break;
		}
		counter++;
	...
	case 2:
	{
		...
		switch (value) {
		case 0:SetClanTag("e        ", ""); break;
		case 1:SetClanTag("ve       ", ""); break;
		case 2:SetClanTag("ave      ", ""); break;
		case 3:SetClanTag("Wave     ", ""); break;
		case 4:SetClanTag(" Wave    ", ""); break;
		case 5:SetClanTag("  Wave   ", ""); break;
		case 6:SetClanTag("   Wave  ", ""); break;
		case 7:SetClanTag("    Wave ", ""); break;
		case 8:SetClanTag("     Wave", ""); break;
		case 9:SetClanTag("      Wav", ""); break;
		case 10:SetClanTag("       Wa", ""); break;
		case 11:SetClanTag("        W", ""); break;
		case 12:SetClanTag("         ", ""); break;
		}
		counter++;
...
void __fastcall PaintTraverse_Hooked(PVOID pPanels, int edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	else if (FocusOverlayPanel == vguiPanel)
	{
		...
		float AsRed = g_menu::window.ColorsTab.ACOLOR_r.GetValue() / 100;
		float AsGreen = g_menu::window.ColorsTab.ACOLOR_g.GetValue() / 100;
		float AsBlue = g_menu::window.ColorsTab.ACOLOR_b.GetValue() / 100;

		float WaRed = g_menu::window.ColorsTab.WACOLOR_r.GetValue() / 100;
		float WaGreen = g_menu::window.ColorsTab.WACOLOR_g.GetValue() / 100;
		float WaBlue = g_menu::window.ColorsTab.WACOLOR_b.GetValue() / 100;

		float amountr = g_menu::window.VisualsTab.colmod.GetValue() / 100;

		float asusamountr = g_menu::window.VisualsTab.asusamount.GetValue() / 100;
...

				{
					...
					for (MaterialHandle_t i = Interfaces::MaterialSystem->FirstMaterial(); i != Interfaces::MaterialSystem->InvalidMaterial(); i = Interfaces::MaterialSystem->NextMaterial(i))
					{
						...
						*(float*)((DWORD)&NightSkybox1->fnChangeCallback + 0xC) = NULL;
						...
						{
							pMaterial->ColorModulation(0.4, 0.4, 0.4);
						}
						...
						if (g_menu::window.VisualsTab.AsWaEnable.GetState())
						{
							...
							{
								pMaterial->ColorModulate(WaRed, WaGreen, WaBlue);
							}

							if (!strcmp(pMaterial->GetTextureGroupName(), "StaticProp textures"))
							{
								pMaterial->AlphaModulate(asusamountr);
								pMaterial->ColorModulate(AsRed, AsGreen, AsBlue);
							}
		...
		}
	    {
			...
			Render::Clear(11, 13, 160, 18, Color(0, 255, 0, 155));
			Render::Text(15, 15, Color(0, 0, 0, 255), Render::Fonts::supremacy, (" Wave Ware  |"));
			Render::Textf(80, 15, Color(0, 0, 0, 255), Render::Fonts::supremacy, ("|  Best 4 HvH"), GetTimeString().c_str());
		}
		...
		if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame() && g_menu::window.SettingsTab.OtherHitmarker.GetIndex())
...
void __fastcall  hkSceneEnd(void *pEcx, void *pEdx) {
	 ...
	 if (g_menu::window.VisualsTab.FakeAngleChams.GetState())
	{
		IClientEntity* pLocal = Interfaces::EntList->GetClientEntity(Interfaces::Engine->GetLocalPlayer());
		if (pLocal)
		{
			...
			if (CoveredLit)
			{
				...
				pLocal->draw_model(STUDIO_RENDER, 140);
...
bool __fastcall Hooked_FireEventClientSide(PVOID ECX, PVOID EDX, IGameEvent *Event)
{
	...
	if (g_menu::window.RageBotTab.AimbotEnable.GetState())
	{
		...
		{
			if (strcmp(Event->GetName(), "round_start") == 0)
				Interfaces::Engine->ClientCmd_Unrestricted("buy defuser; buy vest; buy vesthelm; buy taser; buy molotov; buy smokegrenade; buy hegrenade;");
		}
	}
			...
			if ((g_menu::window.VisualsTab.FiltersAll.GetState() || pModelEntity->GetTeamNum() != pLocal->GetTeamNum()))
			{
				...
				if (pModelEntity->IsAlive() && pModelEntity->GetHealth() > 0 /*&& pModelEntity->GetTeamNum() != local->GetTeamNum()*/)
				{
					...
					if (pModelEntity->GetTeamNum() != pLocal->GetTeamNum())
					{
						...
						flColor[0] = g_menu::window.ColorsTab.TColorNoVisR.GetValue() / 255.f;
						flColor[1] = g_menu::window.ColorsTab.TColorNoVisG.GetValue() / 255.f;
						flColor[2] = g_menu::window.ColorsTab.TColorNoVisB.GetValue() / 255.f;
					...
					{
						flColor[0] = g_menu::window.ColorsTab.TColorVisR.GetValue() / 255.f;
						flColor[1] = g_menu::window.ColorsTab.TColorVisG.GetValue() / 255.f;
						flColor[2] = g_menu::window.ColorsTab.TColorVisB.GetValue() / 255.f;
			...
			if (pEntity)
			{
				...
				if (pOwner)
				{
					if (pOwner == pLocal)
					{
						...
						if (g_menu::window.CSkinsTab.SkinsEnable.GetState())
						{
							int Model = g_menu::window.CSkinsTab.KnifeModel.GetIndex();

							int M41S = g_menu::window.CSkinsTab.M41SSkin.GetIndex();
							int M4A4 = g_menu::window.CSkinsTab.M4A4Skin.GetIndex();
							int AK47 = g_menu::window.CSkinsTab.AK47Skin.GetIndex();
							int AWP = g_menu::window.CSkinsTab.AWPSkin.GetIndex();
							int GLOCK = g_menu::window.CSkinsTab.GLOCKSkin.GetIndex();
							int USPS = g_menu::window.CSkinsTab.USPSSkin.GetIndex();
							int DEAGLE = g_menu::window.CSkinsTab.DEAGLESkin.GetIndex();
							int FIVE7 = g_menu::window.CSkinsTab.FIVESEVENSkin.GetIndex();
							int AUG = g_menu::window.CSkinsTab.AUGSkin.GetIndex();
							int FAMAS = g_menu::window.CSkinsTab.FAMASSkin.GetIndex();
							int G3SG1 = g_menu::window.CSkinsTab.G3SG1Skin.GetIndex();
							int Galil = g_menu::window.CSkinsTab.GALILSkin.GetIndex();
							int M249 = g_menu::window.CSkinsTab.M249Skin.GetIndex();
							int MAC10 = g_menu::window.CSkinsTab.MAC10Skin.GetIndex();
							int P90 = g_menu::window.CSkinsTab.P90Skin.GetIndex();
							int UMP45 = g_menu::window.CSkinsTab.UMP45Skin.GetIndex();
							int XM1014 = g_menu::window.CSkinsTab.XM1014Skin.GetIndex();
							int BIZON = g_menu::window.CSkinsTab.BIZONSkin.GetIndex();
							int MAG7 = g_menu::window.CSkinsTab.MAG7Skin.GetIndex();
							int NEGEV = g_menu::window.CSkinsTab.NEGEVSkin.GetIndex();
							int SAWEDOFF = g_menu::window.CSkinsTab.SAWEDOFFSkin.GetIndex();
							int TEC9 = g_menu::window.CSkinsTab.TECNINESkin.GetIndex();
							int P2000 = g_menu::window.CSkinsTab.P2000Skin.GetIndex();
							int MP7 = g_menu::window.CSkinsTab.MP7Skin.GetIndex();
							int MP9 = g_menu::window.CSkinsTab.MP9Skin.GetIndex();
							int NOVA = g_menu::window.CSkinsTab.NOVASkin.GetIndex();
							int P250 = g_menu::window.CSkinsTab.P250Skin.GetIndex();
							int SCAR20 = g_menu::window.CSkinsTab.SCAR20Skin.GetIndex();
							int SG553 = g_menu::window.CSkinsTab.SG553Skin.GetIndex();
							int SSG08 = g_menu::window.CSkinsTab.SSG08Skin.GetIndex();
							int Magnum = g_menu::window.CSkinsTab.r8skin.GetIndex();
							int CZ = g_menu::window.CSkinsTab.CZSkin.GetIndex();
							int dualies = CZ = g_menu::window.CSkinsTab.DUALSSkin.GetIndex();
								...
								if (pEntity->GetClientClass()->m_ClassID == (int)CSGOClassID::CKnife)
								{
									...
									if (Model == 0) // Bayonet
									{
										...
										*pWeapon->m_AttributeManager()->m_Item()->ItemDefinitionIndex() = 500;
										...
										int Skin = g_menu::window.CSkinsTab.KnifeSkin.GetIndex();
