...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	Render::Clear(window->m_x - 8, window->m_y - 8, window->m_iWidth + 16, window->m_iHeight + 16, Color(10, 10, 10, 255));
	...
	if (TabCount) // If there are some tabs
	{
		for (int i = 0; i < TabCount; i++)
		{
			...
			if (window->SelectedTab == tab) {
				...
				Render::Text(TabDrawArea.left + 12, TabDrawArea.top, Color(0, 209, 255, 255), Render::Fonts::supremacy, tab->Title.c_str());
			}
			else {
				xAxis = window->m_x + 8 - (45 + TextSize.right / 2);
