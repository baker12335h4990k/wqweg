...
void FakeWalk(CUserCmd * pCmd, bool & bSendPacket)
{
	...
	if (GetAsyncKeyState(VK_SHIFT))
	{
		...
		if (iChoked < 1)
		{
			...
			pCmd->command_number += 9.07 + pCmd->tick_count % 2 ? 0 : 1; // 5
...
void CRageBot::DoAimbot(CUserCmd *pCmd, bool &bSendPacket)
{
	...
	if (g_menu::window.RageBotTab.AutoHitChance.GetState())
	{
		...
		{
			g_menu::window.RageBotTab.AccuracyHitchance.SetValue(45);
		}
		...
		{
			g_menu::window.RageBotTab.AccuracyHitchance.SetValue(80);
		}
		...
		{
			g_menu::window.RageBotTab.AccuracyHitchance.SetValue(18);
		}
	...
	if (g_menu::window.RageBotTab.AutoMinimumDamage.GetState())
	{
		if (GameUtils::IsSSG08(pWeapon))
		{
			g_menu::window.RageBotTab.AccuracyMinimumDamage.SetValue(51);
		}

		else if (GameUtils::IsAK47(pWeapon))
		{
			g_menu::window.RageBotTab.AccuracyMinimumDamage.SetValue(11);
		}

		else if (GameUtils::IsDEAGLE(pWeapon))
		{
			g_menu::window.RageBotTab.AccuracyMinimumDamage.SetValue(45);
		}

		else if (GameUtils::IsAutoSniper(pWeapon))
		{
			g_menu::window.RageBotTab.AccuracyMinimumDamage.SetValue(14);
		}
		...
		{
			g_menu::window.RageBotTab.AccuracyMinimumDamage.SetValue(99);
		}
